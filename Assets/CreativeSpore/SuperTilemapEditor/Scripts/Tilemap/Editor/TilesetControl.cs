...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilesetControl
    {
        ...
        private GUIStyle m_toggleButtonDisableStyle;
        ...
        private List<int> m_visibleTileList = new List<int>();
        private int m_tileViewRowLength = 1;
        private Rect m_rTileScrollSize;
        private float m_lastTime;
        private float m_timeDt;
        private bool m_wasMouseDownInsideTileScroll = false;
        ...
        public void Display()
        {
            ...
            if(m_toggleButtonDisableStyle == null)
            {
                m_toggleButtonDisableStyle = new GUIStyle("Button");
                m_toggleButtonDisableStyle.normal.background = m_toggleButtonDisableStyle.active.background;
            }
            ...
            m_tilesScrollPos = EditorGUILayout.BeginScrollView(m_tilesScrollPos, m_scrollStyle);
            {
                ...
                if (Tileset.Tiles != null)
                {
                    ...
                    List<int> visibleTileList = new List<int>();
                    int tileViewWidth = m_viewMode == eViewMode.Tileset ? Tileset.Width : tileView.tileSelection.rowLength;
                    int tileViewHeight = m_viewMode == eViewMode.Tileset ? Tileset.Height : ((tileView.tileSelection.tileIds.Count - 1) / tileView.tileSelection.rowLength) + 1;
                    ...
                    for (int i = 0; i < totalCount; ++i)
                    {
                        ...
                        //---
                        {
                            ...
                            if (isLeftMouseReleased && m_wasMouseDownInsideTileScroll && rVisualTile.Contains(e.mousePosition)
            ...
            {
                m_rTileScrollSize = GUILayoutUtility.GetLastRect();
            ...
            if( GUILayout.Button("Display List", m_displayBrushReordList? m_toggleButtonDisableStyle : (GUIStyle)"Button"))
            {
                m_displayBrushReordList = !m_displayBrushReordList;
            }
                ...
                m_brushesScrollPos = EditorGUILayout.BeginScrollView(m_brushesScrollPos, m_scrollStyle);
                {
                    ...
                    if (Tileset.Brushes != null)
                    {
                        ...
                        for (int i = 0; i < Tileset.Brushes.Count; ++i)
                        {
                            ...
                            int tileId = brushCont.BrushAsset.PreviewTileId();
                            Rect tileUV = default(Rect);
                            if (brushCont.BrushAsset.IsAnimated())
                            {
                                tileUV = brushCont.BrushAsset.GetAnimUV();
                            }
                            else if (tileId != Tileset.k_TileId_Empty)
                            {
                                tileUV = Tileset.Tiles[tileId].uv;
                            }
                            GUI.DrawTextureWithTexCoords(rVisualTile, Tileset.AtlasTexture, tileUV, true);
                            if ((isLeftMouseReleased || m_dblClick.IsDblClick) && rVisualTile.Contains(Event.current.mousePosition))
                            {
            ...
            {
                m_wasMouseDownInsideTileScroll = true;
            }
            else if (e.type == EventType.MouseUp)
            {
                m_wasMouseDownInsideTileScroll = false;
            }
        }
            ...
            if (m_startDragTileIdxRect.Key != m_endDragTileIdxRect.Key)
            {
                ...
                for (int ty = ty_end; ty >= ty_start; --ty) // NOTE: this goes from bottom to top to fit the tilemap coordinate system
                {
                    for (int tx = tx_start; tx <= tx_end; ++tx, ++tileIdx)
                    {
                        ...
                        int tileId = m_visibleTileList[visibleTileIdx];
                        tileIds.Add(tileId);
                    }
                }
                Tileset.TileSelection = new TileSelection(tileIds, tx_end - tx_start + 1);
