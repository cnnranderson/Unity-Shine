...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilePropertiesControl
    {
        ...
        private void DisplayParameters()
        {
            ...
            else
            {
                ...
                if (m_parameterListOwner != null)
                {
                    ...
                    GUILayoutUtility.GetRect(1, 1, GUILayout.Width(Tileset.VisualTileSize.x), GUILayout.Height(Tileset.VisualTileSize.y));
                    Rect tileUV = m_parameterListOwner is Tileset ? 
                        Tileset.Tiles[((Tileset)m_parameterListOwner).SelectedTileId].uv 
                        :
                        ((TilesetBrush)m_parameterListOwner).IsAnimated() ? 
                            ((TilesetBrush)m_parameterListOwner).GetAnimUV() 
                            : 
                            Tileset.Tiles[((TilesetBrush)m_parameterListOwner).PreviewTileId()].uv;
                    GUI.color = Tileset.BackgroundColor;
                    GUI.DrawTextureWithTexCoords(GUILayoutUtility.GetLastRect(), EditorGUIUtility.whiteTexture, tileUV, true);
                    GUI.color = Color.white;
                    GUI.DrawTextureWithTexCoords(GUILayoutUtility.GetLastRect(), Tileset.AtlasTexture, tileUV, true);
        ...
        private GUIStyle m_colliderBgStyle;
        private GUIStyle m_collVertexHandleStyle;
        private GUIStyle m_vertexCoordStyle;
        ...
        private void DisplayCollider()
        {
            ...
            if (m_colliderBgStyle == null)
            {
                m_colliderBgStyle = new GUIStyle("ScrollView");
            }

            if (m_collVertexHandleStyle == null || m_collVertexHandleStyle.normal.background == null)
            {
                m_collVertexHandleStyle = new GUIStyle("U2D.dragDot");//"MuteToggle"; ToggleMixed
                m_collVertexHandleStyle.normal.textColor = Color.cyan;
                m_collVertexHandleStyle.contentOffset = Vector2.one * 8;
            }

            if (m_vertexCoordStyle == null)
            {
                m_vertexCoordStyle = new GUIStyle("Label");
                m_vertexCoordStyle.normal.textColor = Color.cyan;
                m_vertexCoordStyle.fontStyle = FontStyle.Bold;
            }
            ...
            if (m_prevBgColor != Tileset.BackgroundColor || m_colliderBgStyle.normal.background == null)
            {
                m_prevBgColor = Tileset.BackgroundColor;
                if (m_colliderBgStyle.normal.background == null) m_colliderBgStyle.normal.background = new Texture2D(1, 1) { hideFlags = HideFlags.DontSave };
                m_colliderBgStyle.normal.background.SetPixel(0, 0, Tileset.BackgroundColor);
                m_colliderBgStyle.normal.background.Apply();
            }

            Tile selectedTile = isMultiselection ? Tileset.Tiles[Tileset.TileSelection.tileIds[0]] : Tileset.SelectedTile;
            ...
            GUI.BeginGroup(rCollArea, m_colliderBgStyle);
            ...
            GUI.DrawTextureWithTexCoords(m_rTile, Tileset.AtlasTexture, tileUV);
                ...
                if (selectedTile.collData.type == eTileCollider.Polygon)
                {
                    ...
                    for (int i = 0; i < collVertices.Length; ++i)
                    {
                        ...
                        else
                        {
                            ...
                            if (dist <= minDist && dist < m_collVertexHandleStyle.normal.background.width)
                            {
                                minDist = dist;
                                m_activeVertexIdx = i;
                            }
                        ...
                        if (e.type == EventType.Repaint)
                        {
                            if (i == m_activeVertexIdx)
                            {
                                m_vertexCoordStyle.fontSize = (int)(pixelSize);
                                GUI.Label(new Rect(m_rTile.x, m_rTile.yMax, m_rTile.width, 3 * pixelSize), Vector2.Scale(collVertices[i], Tileset.TilePxSize).ToString(), m_vertexCoordStyle);
                            }
                            GUI.color = m_activeVertexIdx == i ? (isRemovingVertexOn ? Color.red : Color.cyan) : new Color(0.7f, 0.7f, 0.7f, 0.8f);
                            m_collVertexHandleStyle.Draw(new Rect(s0.x - m_collVertexHandleStyle.normal.background.width / 2, s0.y - m_collVertexHandleStyle.normal.background.height / 2, 1, 1), i.ToString(), false, false, false, false);
                            GUI.color = Color.white;
                        }
                    }
                    ...
                    if (isAddingVertexOn)
                    {
                        ...
                        else if (e.type == EventType.Repaint)
                        {
                            ...
                            m_collVertexHandleStyle.Draw(new Rect(newVertexPos.x - m_collVertexHandleStyle.normal.background.width / 2, newVertexPos.y - m_collVertexHandleStyle.normal.background.height / 2, 1, 1), segmentIdx.ToString(), false, false, false, false);
                            GUI.color = Color.white;
                        }
            ...
            if (saveChanges)
            {
                if (isMultiselection)
                {
                    for (int i = 0; i < Tileset.TileSelection.tileIds.Count; ++i)
                    {
                        Tileset.Tiles[Tileset.TileSelection.tileIds[i]].collData = selectedTile.collData.Clone();
                    }
                }
                EditorUtility.SetDirty(Tileset);
            }
