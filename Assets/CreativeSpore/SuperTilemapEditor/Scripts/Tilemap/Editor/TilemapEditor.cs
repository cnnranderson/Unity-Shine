...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilemapEditor : Editor
    {
        ...
        private void OnTileSelectionChanged(Tileset source)
        {
            ...
            {
                for (int i = 0; i < source.TileSelection.tileIds.Count; ++i)
                {
                    int gx = i % source.TileSelection.rowLength;
                    int gy = i / source.TileSelection.rowLength;
                    brush.BrushTilemap.SetTileData(gx, gy, (uint)source.TileSelection.tileIds[i]);
                }
            }
            brush.BrushTilemap.UpdateMesh();
            brush.Offset = Vector2.zero;
        ...
        private GUIStyle m_mapEditEnabledStyle;
        private GUIStyle m_mapEditDisabledStyle;
        [SerializeField]
        private bool m_toggleMapBoundsEdit = false;

        public override void OnInspectorGUI()
        {
            if (m_mapEditEnabledStyle == null)
            {
                m_mapEditEnabledStyle = new GUIStyle("Button");
                m_mapEditDisabledStyle = new GUIStyle("Button");
                m_mapEditDisabledStyle.normal.background = m_mapEditDisabledStyle.active.background;
            }

            serializedObject.Update();

            Tileset prevTileset = m_tilemap.Tileset;
            m_tilemap.Tileset = (Tileset)EditorGUILayout.ObjectField(m_tilemap.Tileset, typeof(Tileset), false);
            ...
            if (s_editMode == eEditMode.Renderer)
            {
                ...
                */
                ...
                EditorGUI.BeginChangeCheck();
                int orderInLayer = EditorGUILayout.IntField("Order In Layer", m_tilemap.OrderInLayer);
                if (EditorGUI.EndChangeCheck())
                {
                    m_tilemap.OrderInLayer = orderInLayer; //NOTE: this makes all chunks to be modified as well
                }                
                //---            

                EditorGUILayout.PropertyField(serializedObject.FindProperty("InnerPadding"));
            ...
            else if (s_editMode == eEditMode.Map)
            {
                ...
                EditorGUILayout.PrefixLabel("Edit Map Bounds");
                if (GUILayout.Button(EditorGUIUtility.FindTexture("EditCollider"), m_toggleMapBoundsEdit ? m_mapEditDisabledStyle : m_mapEditEnabledStyle, GUILayout.Width(30)))
                {
                    m_toggleMapBoundsEdit = !m_toggleMapBoundsEdit;
                }
                ...
                if (GUILayout.Button("Shrink to Visible Area"))
                {
                ...
                {
                    m_tilemap.Refresh(true, true, true);
                }
        ...
        bool DoToolBar()
        {
            ...
            Rect rTools = new Rect(4f, 4f, Mathf.Max(m_toolbarBoxStyle.CalcSize(brushCoords).x, m_toolbarBoxStyle.CalcSize(selectedTileOrBrushId).x) + 4f, 44f);
            ...
            {
                GUILayout.Label(selectedTileOrBrushId, m_toolbarBoxStyle);
            }
            GUILayout.Label("<b> F1 - Display Help</b>", m_toolbarBoxStyle);
            GUILayout.EndArea();

            // Display ToolBar
            Rect rToolBar = new Rect(rTools.xMax + 4f, rTools.y, 224, 32f);
        ...
        void DisplayHelpBox()
        {
            ...
            Handles.BeginGUI();
            Rect rHelpBox = new Rect(new Vector2(2f, 50f), m_toolbarBoxStyle.CalcSize(helpContent));
